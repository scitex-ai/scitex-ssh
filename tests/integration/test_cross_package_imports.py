"""Cross-package import gate (PS-140) for scitex-ssh.
...
Regenerate with:
    ...
    scitex-dev ecosystem install-cross-package-gate scitex-ssh --force
...
def test_cross_package_import_resolves(module_name):
    ...
    # Assert — kept STRONGER than the generator's default `is not None`.
    # The tail below the closing sentinel is preserved byte-identically
    # across regeneration precisely so a leaf can strengthen its own
    # assertion in place; identity-checking `__name__` is what the
    # previous hand-written gate asserted and it still catches a module
    # object that resolved to something other than the name asked for.
