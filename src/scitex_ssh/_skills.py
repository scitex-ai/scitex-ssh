"""`scitex-ssh skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-ssh/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-ssh/` directory."""
    import scitex_ssh

    pkg_dir = Path(scitex_ssh.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-ssh/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-ssh.

    \b
    Examples:
      $ scitex-ssh skills list
      $ scitex-ssh skills get 01_installation
      $ scitex-ssh skills install                  # → ~/.scitex/dev/skills/scitex-ssh/
      $ scitex-ssh skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-ssh skills get 01_installation
      $ scitex-ssh skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-ssh/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-ssh/` dir to
    ~/.scitex/dev/skills/scitex-ssh/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-ssh skills install
      $ scitex-ssh skills install --claude-symlink
      $ scitex-ssh skills install --no-link --dest /tmp/scitex-ssh-skills
    """
